...
int main() {
	...
	int flag_green = 0;
	int flag_yellow = 0;
	char msg[50], random_char[50], msg_to_display[50], char_time_pressed_green[50], char_time_pressed_yellow[50], char_score_green[50], char_score_yellow[50], char_score[50];
	...
	while(true) {
		...
		while(flag_green == 0 || flag_yellow == 0) { 
			...
			if(flag_green == 1 && flag_yellow == 1){
				...
				if(abs(time_pressed_green-random) == abs(time_pressed_yellow-random)){
					...
					lcdClear(lcdHandle);
					strcpy(msg_to_display, "");
					strcpy(char_time_pressed_green, "");
					strcpy(char_time_pressed_yellow, "");
					if(abs(time_pressed_green-random) < abs(time_pressed_yellow-random)){
						points_player1++;
						strcpy(msg_to_display, "1pt goes to     GREEN! ");
						sprintf(char_time_pressed_green, "%d", time_pressed_green*10);
						strcat(msg_to_display, char_time_pressed_green);
						strcat(msg_to_display, "ms");
						lcdPuts(lcdHandle, msg_to_display);
					}
					if(abs(time_pressed_green-random) > abs(time_pressed_yellow-random)){
						points_player2++;
						strcpy(msg_to_display, "1pt goes to     YELLOW! ");
						sprintf(char_time_pressed_yellow, "%d", time_pressed_yellow*10);
						strcat(msg_to_display, char_time_pressed_yellow);
						strcat(msg_to_display, "ms");
						lcdPuts(lcdHandle, msg_to_display);
					}
				}
		...
		**********************************/
		if(level == 3){
			...
			strcpy(char_score_green, "");
			sprintf(char_score_green, "%d", points_player1);
			strcpy(char_score_yellow, "");
			sprintf(char_score_yellow, "%d", points_player2);
			strcpy(char_score, char_score_green);
			strcat(char_score, " VS ");
			strcat(char_score, char_score_yellow);
			lcdPuts(lcdHandle, char_score);
			exit(0);
		}
	}
